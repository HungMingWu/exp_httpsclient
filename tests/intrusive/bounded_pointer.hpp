...
class bounded_pointer
{
   ...
   public:
   using mut_val_t = typename std::remove_const_t<T>;
   using const_val_t = const mut_val_t;

   using reference = bounded_reference<T>;

   static const unsigned char max_offset = (unsigned char)-1;

   bounded_pointer() = default;
   bounded_pointer(const bounded_pointer& other) = default;
   ...
   bounded_pointer& operator = (const bounded_pointer& other) = default;

   template <class T2>
   bounded_pointer& operator= (const bounded_pointer<T2> & other)
   requires (std::is_convertible_v<T2*, T*>)
   {  m_offset = other.m_offset;  return *this;  }

   const bounded_pointer<typename std::remove_const_t<T>>& unconst() const
   { return *reinterpret_cast< const bounded_pointer<typename std::remove_const_t<T> >* >(this); }

   bounded_pointer< typename std::remove_const_t<T>>& unconst()
   { return *reinterpret_cast< bounded_pointer< typename std::remove_const_t<T>>* >(this); }
   ...
   unsigned char m_offset = max_offset;
}; // class bounded_pointer
...
{
   public:
   typedef typename std::remove_const_t<T> mut_val_t;
   ...
   template <class T2>
   bounded_reference& operator= (const bounded_reference<T2> & other)
   requires (std::is_convertible_v<T2*, T*>)
   {  m_offset = other.m_offset;  return *this;  }
...
class bounded_allocator
{
   ...
   static void init()
   {
      ...
      m_in_use = std::vector<bool>(max_offset, false);
   ...
   static T* m_base;
   static std::vector<bool> m_in_use;
...
template < typename T >
std::vector<bool> bounded_allocator<T>::m_in_use;
...
class bounded_pointer_holder
{
   public:
   using value_type = T;
   using pointer = bounded_pointer<value_type>;
   using const_pointer = bounded_pointer<const value_type>;
   using allocator_type = bounded_allocator<value_type>;
