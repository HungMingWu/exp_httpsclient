...
struct test_unordered
{
   ...
   static void test_insert(value_cont_type& values, std::true_type);
   static void test_insert(value_cont_type& values, std::false_type);
   static void test_swap(value_cont_type& values);
   static void test_rehash(value_cont_type& values, std::true_type);
   static void test_rehash(value_cont_type& values, std::false_type);
...
void test_unordered<ContainerDefiner>::test_all (value_cont_type& values)
{
   ...
   typedef typename unordered_type::bucket_ptr    bucket_ptr;
   {
      ...
      testset.clear();
      testset.insert(values.begin(), values.end());
      typedef std::bool_constant<boost::intrusive::test::is_multikey_true
   ...
   test_sort(values);
   test_insert(values, std::bool_constant<boost::intrusive::test::is_multikey_true<unordered_type>::value>());
   test_swap(values);
   test_rehash(values, std::bool_constant<unordered_type::incremental>());
   test_find(values);
   test_impl();
   test_clone(values);
}
...
{
   using unordered_type = typename ContainerDefiner::template container<>::type;
   using bucket_traits = typename unordered_type::bucket_traits;
   using bucket_ptr = typename unordered_type::bucket_ptr;
...
template<class ContainerDefiner>
void test_unordered<ContainerDefiner>::test_insert(value_cont_type& values, std::false_type) //not multikey
{
   ...
   REQUIRE ((!testset.insert_check(key_of_value()(values[2]), commit_data).second));
   REQUIRE (( testset.insert_check(key_of_value()(values[0]), commit_data).second));
...
template<class ContainerDefiner>
void test_unordered<ContainerDefiner>::test_insert(value_cont_type& values, std::true_type) //is multikey
{
   typedef typename ContainerDefiner::template container
      <>::type unordered_type;
   ...
   typedef typename unordered_type::key_type key_type;
   {
      ...
      if(unordered_type::incremental){
         {
            ...
            REQUIRE (testset.erase(key_type(1)) == 1);
            REQUIRE (testset.erase(key_type(2)) == 2);
            REQUIRE (testset.erase(key_type(3)) == 1);
            REQUIRE (testset.erase(key_type(4)) == 1);
            REQUIRE (testset.erase(key_type(5)) == 1);
            REQUIRE (testset.empty() == true);
            ...
            REQUIRE (testset2.erase(key_type(5)) == 1);
            REQUIRE (testset2.erase(key_type(2)) == 2);
            REQUIRE (testset2.erase(key_type(1)) == 1);
            REQUIRE (testset2.erase(key_type(4)) == 1);
            REQUIRE (testset2.erase(key_type(3)) == 1);
            REQUIRE (testset2.empty() == true);
         }
      }
      else{
         {
      ...
      }
      {
         ...
         for(unsigned int i = 0, max = random_size; i != max; ++i){
            ...
            REQUIRE (testset_new.size() == (max -1));
         }
      }
   }
   {
      ...
      for(unsigned int initial_pos = 0; initial_pos != (NumIterations+1); ++initial_pos){
         for(unsigned int final_pos = initial_pos; final_pos != (NumIterations+1); ++final_pos){
            ...
            REQUIRE (testset.size() == random_init.size());
            ...
            REQUIRE (testset.size() == (random_init.size()-(final_pos - initial_pos)));

            //Now test...
            REQUIRE ((random_init.size() - erased_cnt) == testset.size());
...
void test_unordered<ContainerDefiner>::test_swap(value_cont_type& values)
{
   ...
   if(unordered_type::incremental){
      ...
      REQUIRE (&*testset1.begin() == &values[2]);
   }
   else{
...
template<class ContainerDefiner>
void test_unordered<ContainerDefiner>::test_rehash(value_cont_type& values, std::true_type)
{
   typedef typename ContainerDefiner::template container
      <>::type unordered_type;
   ...
   //Test current state
   REQUIRE(testset1.split_count() == BucketSize/2);
   {  int init_values [] = { 4, 5, 1, 2, 2, 3 };
      TEST_INTRUSIVE_SEQUENCE_MAYBEUNIQUE( init_values, testset1 );  }
   //Incremental rehash step
   REQUIRE (testset1.incremental_rehash() == true);
   REQUIRE(testset1.split_count() == (BucketSize/2+1));
   ...
   for(std::size_t split_bucket = testset1.split_count(); split_bucket != BucketSize; ++split_bucket){
      REQUIRE (testset1.incremental_rehash() == true);
      REQUIRE(testset1.split_count() == (split_bucket+1));
      {  int init_values [] = { 1, 2, 2, 3, 4, 5 };
      TEST_INTRUSIVE_SEQUENCE_MAYBEUNIQUE( init_values, testset1 );  }
   }
   //This incremental rehash should fail because we've reached the end of the bucket array
   REQUIRE (testset1.incremental_rehash() == false);
   REQUIRE(testset1.split_count() == BucketSize);
   {  int init_values [] = { 1, 2, 2, 3, 4, 5 };
   TEST_INTRUSIVE_SEQUENCE_MAYBEUNIQUE( init_values, testset1 );  }
   ...
   REQUIRE(testset1.incremental_rehash(bucket_traits(
   ...
   for(std::size_t split_bucket = testset1.split_count(); split_bucket > 6; --split_bucket){
      REQUIRE (testset1.incremental_rehash(false) == true);
      REQUIRE(testset1.split_count() == (split_bucket-1));
      {  int init_values [] = { 1, 2, 2, 3, 4, 5 };
      TEST_INTRUSIVE_SEQUENCE_MAYBEUNIQUE( init_values, testset1 );  }
   }
   ...
   REQUIRE(testset1.incremental_rehash(false) == false);
   REQUIRE(testset1.split_count() == BucketSize/2);
   {  int init_values [] = { 4, 5, 1, 2, 2, 3 };
   TEST_INTRUSIVE_SEQUENCE_MAYBEUNIQUE( init_values, testset1 );  }
}
template<class ContainerDefiner>
void test_unordered<ContainerDefiner>::test_rehash(value_cont_type& values, std::false_type)
{
   typedef typename ContainerDefiner::template container
      <>::type unordered_type;
...
void test_unordered<ContainerDefiner>::test_find(value_cont_type& values)
{
   ...
   value_type cmp_val;
   cmp_val.value_ = 2;
   REQUIRE (testset.count(key_of_value()(cmp_val)) == (is_multikey ? 2 : 1));
   ...
   REQUIRE (boost::intrusive::iterator_distance (range.first, range.second) == (is_multikey ? 2 : 1));
   cmp_val.value_ = 7;
   REQUIRE (testset.find (key_of_value()(cmp_val)) == testset.end());
   REQUIRE (testset.count(key_of_value()(cmp_val)) == 0);
}
