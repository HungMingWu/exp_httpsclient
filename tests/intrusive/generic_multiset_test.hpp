...
void test_generic_multiset<ContainerDefiner>::test_merge(value_cont_type& values)
{
   ...
   REQUIRE (*testset2.find(key_of_value()(cmp_val)) == values[5]);
   REQUIRE (*testset2.find(2, any_greater()) == values[5]);
   REQUIRE (&*(++testset2.find(key_of_value()(cmp_val))) == &values[1]);

   testset1.merge(testset2);
   testset1.check();
   testset2.check();

   REQUIRE (testset1.size() == 6);
   {  int init_values [] = { 1, 2, 2, 3, 4, 5 };
      TEST_INTRUSIVE_SEQUENCE( init_values, testset1.begin() );  }
   REQUIRE (*testset1.find(key_of_value()(cmp_val)) == values[5]);
   REQUIRE (*testset1.find(2, any_less()) == values[5]);
   REQUIRE (&*(++testset1.find(key_of_value()(cmp_val))) == &values[1]);
...
void test_generic_multiset<ContainerDefiner>::test_find(value_cont_type& values)
{
   ...
   {  //1, 2, 2, 3, 4, 5
      ...
      REQUIRE (range.second->value_ == 3);
      REQUIRE (boost::intrusive::iterator_distance (range.first, range.second) == 3);
      }
      {
      (&cmp_val_lower)->value_ = 1;
      (&cmp_val_upper)->value_ = 2;
      ...
      REQUIRE (boost::intrusive::iterator_distance (const_range.first, const_range.second) == 2);
      }
      {
      (&cmp_val_lower)->value_ = 1;
      (&cmp_val_upper)->value_ = 2;
