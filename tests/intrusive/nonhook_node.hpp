...
struct nonhook_node_member : public NodeTraits::node
{
   ...
   nonhook_node_member()
   {
      node_algorithms::init(std::pointer_traits<node_ptr>::pointer_to(static_cast< node& >(*this)));
   }

   nonhook_node_member(const nonhook_node_member& rhs)
   {
      REQUIRE(!rhs.is_linked());
      node_algorithms::init(std::pointer_traits<node_ptr>::pointer_to(static_cast< node& >(*this)));
   }

   nonhook_node_member& operator = (const nonhook_node_member& rhs)
   {
      REQUIRE(!this->is_linked());
      REQUIRE(!rhs.is_linked());
      return *this;
   }
   ...
   {
      node_algorithms::swap_nodes(std::pointer_traits<node_ptr>::pointer_to(static_cast< node& >(*this)),
                                  std::pointer_traits<node_ptr>::pointer_to(static_cast< node& >(other)));
   }

   bool is_linked() const
   {
      return !node_algorithms::unique(std::pointer_traits<const_node_ptr>::pointer_to(static_cast< const node& >(*this)));
   }
};
...
struct nonhook_node_member_value_traits
{
   ...
   static_assert(Link_Mode == link_mode_type::safe_link || Link_Mode == link_mode_type::auto_unlink);

   static node_ptr to_node_ptr(reference value)
   {
      return std::pointer_traits<node_ptr>::pointer_to(static_cast<node&>(value.*P));
   }

   static const_node_ptr to_node_ptr(const_reference value)
   {
      return std::pointer_traits<const_node_ptr>::pointer_to(static_cast<const node&>(value.*P));
   }
