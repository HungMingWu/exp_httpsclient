...
struct hooks
{
    ...
    typedef slist_base_hook< link_mode<link_mode_type::auto_unlink>
    ...
    typedef slist_member_hook< link_mode<link_mode_type::auto_unlink>
...
struct test_slist
{
    ...
    static void test_back(ValueContainer&, std::true_type);
    static void test_back(ValueContainer&, std::false_type) {}
    ...
    static void test_container_from_end(ValueContainer&, std::true_type);
    static void test_container_from_end(ValueContainer&, std::false_type) {}
};
...
::test_all(ValueContainer& values)
{
    ...
    test_front(values);
    test_back(values, std::bool_constant< list_type::cache_last >());
    test_sort(values);
    test_merge(values);
    ...
    test_swap(values);
    test_clone(values);
    test_container_from_end(values, std::bool_constant< !list_type::linear&& list_type::has_container_from_iterator >());
}
...
template < typename ListType, typename ValueContainer >
void test_slist< ListType, ValueContainer >
::test_back(ValueContainer& values, std::true_type)
{
    list_type testlist;
    REQUIRE(testlist.empty());
    ...
    REQUIRE(*testlist.previous(testlist.end()) == values[1]);
    REQUIRE(&testlist.front() == &values[0]);
    REQUIRE(&testlist.back() == &values[1]);
}
...
::test_slow_insert(ValueContainer& values)
{
    ...
    REQUIRE(testlist.size() == 1);
    REQUIRE(testlist.begin()->value_ == 3);
}
...
::test_swap(ValueContainer& values)
{
    ...
    if (!list_type::linear)
    {
        ...
        REQUIRE(!(&values[1])->is_linked());
        REQUIRE(!(&values[2])->is_linked());
        {  int init_values[] = { 1 };
        TEST_INTRUSIVE_SEQUENCE(init_values, testlist1.begin());  }
...
template < typename ListType, typename ValueContainer >
void test_slist< ListType, ValueContainer >
::test_container_from_end(ValueContainer& values, std::true_type)
{
    list_type testlist1(values.begin(), values.begin() + values.size());
    REQUIRE(testlist1 == list_type::container_from_end_iterator(testlist1.end()));
    REQUIRE(testlist1 == list_type::container_from_end_iterator(testlist1.cend()));
}
...
public:
    int operator()()
    {
        ...
        make_and_test_slist <detail::get_base_value_traits_t
            < value_type
        ...
        //Now linear slists
        make_and_test_slist < detail::get_member_value_traits_t
...
TEST_CASE("slist_test void pointer 1", "slist_test") {
    test_main_template<void*, false, true>()();
}

TEST_CASE("slist_test void pointer 2", "slist_test") {
    test_main_template<void*, true, true>()();
}

TEST_CASE("slist_test smart_ptr 1", "slist_test") {
    test_main_template<boost::intrusive::smart_ptr<void>, false, true>()();
}

TEST_CASE("slist_test smart_ptr 2", "slist_test") {
    test_main_template<boost::intrusive::smart_ptr<void>, true, true>()();
}

TEST_CASE("slist_test bounded_ptr 1", "slist_test") {
    test_main_template_bptr< true >()();
}

TEST_CASE("slist_test bounded_ptr 2", "slist_test") {
