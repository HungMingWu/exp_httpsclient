...
struct treap_prio_types
{
   ...
   using priority_of_value = get_key_of_value_t<VoidOrPrioOfValue, value_type>;
   using priority_type = typename priority_of_value::type;
   using priority_compare = get_prio_comp_t<VoidOrPrioComp, priority_type>;
};
...
class treap_impl
   /// @cond
   : public bstree_impl<ValueTraits, VoidOrKeyOfValue, VoidOrKeyComp, ConstantTimeSize, BsTreeAlgorithms, HeaderHolder>
   /// @endcond
{
   public:
   ...
   [[no_unique_address]] typename treap_prio_types_t::priority_compare functor;
   ...
   typedef typename treap_prio_types_t::priority_type                priority_type;
   typedef typename treap_prio_types_t::priority_of_value            priority_of_value;
   typedef typename treap_prio_types_t::priority_compare             priority_compare;
   ...
   explicit treap_impl( const key_compare &cmp
      ...
      : tree_type(cmp, v_traits), functor(pcmp)
   {}
   ...
   treap_impl(const treap_impl&) = delete;
   treap_impl& operator=(const treap_impl&) = delete;
   //! @copydoc ::boost::intrusive::bstree::bstree(bstree &&)
   treap_impl(treap_impl&& x)
      : tree_type(std::move(x))
      , functor(std::move(x.priv_pcomp()))
   {}

   //! @copydoc ::boost::intrusive::bstree::operator=(bstree &&)
   treap_impl& operator=(treap_impl&& x)
   ...
   static treap_impl &container_from_end_iterator(iterator end_iterator) noexcept;

   //! @copydoc ::boost::intrusive::bstree::container_from_end_iterator(const_iterator)
   static const treap_impl &container_from_end_iterator(const_iterator end_iterator) noexcept;

   //! @copydoc ::boost::intrusive::bstree::container_from_iterator(iterator)
   static treap_impl &container_from_iterator(iterator it) noexcept;

   //! @copydoc ::boost::intrusive::bstree::container_from_iterator(const_iterator)
   static const treap_impl &container_from_iterator(const_iterator it) noexcept;
   ...
   //! <b>Throws</b>: If cloner throws or predicate copy assignment throws. Basic guarantee.
   template <class Cloner, class Disposer>
   void clone_from(treap_impl&& src, Cloner cloner, Disposer disposer)
   {
      tree_type::clone_from(std::move(src), cloner, disposer);
      this->priv_pcomp() = std::move(src.priv_pcomp());
   }
   ...
   {
      insert_commit_data commit_data;
      auto [iter, success] = this->insert_unique_check(hint, key_of_value()(value), priority_of_value()(value), commit_data);
      if (!success)
         return iter;
   ...
   decltype(auto) insert_unique_check
   ...
   std::enable_if_t<!std::is_convertible_v<KeyType, const_iterator>, std::pair<iterator, bool>>
      insert_unique_check
   ...
   void clear_and_dispose(Disposer disposer) noexcept
   {
      node_algorithms::clear_and_dispose(this->tree_type::header_ptr()
         , detail::make_node_disposer<TreapAlgorithms>(disposer, &this->get_value_traits()));
   ...
   void merge_unique(treap_impl
      <ValueTraits, VoidOrKeyOfValue, Compare2, VoidOrPrioOfValue, VoidOrPrioComp, ConstantTimeSize, HeaderHolder> &source)
   #endif
   {
      ...
      >::type
{
   ...
   inline treap(treap&& x) : Base(std::move(x)) {}

   inline treap& operator=(treap&& x)
   {  return static_cast<treap&>(this->Base::operator=(std::move(x)));  }
   ...
   template <class Cloner, class Disposer>
   inline void clone_from(treap&& src, Cloner cloner, Disposer disposer)
   {  Base::clone_from(std::move(src), cloner, disposer);  }

   inline static treap &container_from_end_iterator(iterator end_iterator) noexcept
   {  return static_cast<treap &>(Base::container_from_end_iterator(end_iterator));   }

   inline static const treap &container_from_end_iterator(const_iterator end_iterator) noexcept
   {  return static_cast<const treap &>(Base::container_from_end_iterator(end_iterator));   }

   inline static treap &container_from_iterator(iterator it) noexcept
   {  return static_cast<treap &>(Base::container_from_iterator(it));   }

   inline static const treap &container_from_iterator(const_iterator it) noexcept
