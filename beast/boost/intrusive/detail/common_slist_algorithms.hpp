...
class common_slist_algorithms
{
   ...
   inline static void unlink_after(node_ptr prev_node) noexcept
   {
      const_node_ptr this_node(NodeTraits::get_next(prev_node));
      NodeTraits::set_next(prev_node, NodeTraits::get_next(this_node));
   }

   inline static void unlink_after(node_ptr prev_node, node_ptr last_node) noexcept
   {  NodeTraits::set_next(prev_node, last_node);  }

   inline static void link_after(node_ptr prev_node, node_ptr this_node) noexcept
   {
      NodeTraits::set_next(this_node, NodeTraits::get_next(prev_node));
      NodeTraits::set_next(prev_node, this_node);
   }

   inline static void incorporate_after(node_ptr bp, node_ptr b, node_ptr be)
   {
   ...
   //!
   //! <b>Throws</b>: Nothing.
   static std::size_t distance(const const_node_ptr &f, const const_node_ptr &l) noexcept
   {
      const_node_ptr i(f);
      std::size_t result = 0;
      while(i != l){
