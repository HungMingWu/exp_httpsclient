   ...
   :  public any_slist_node_traits<VoidPointer>
{
   ...
   inline static node_ptr get_prev_in_group(const const_node_ptr & n)
   {  return n->node_ptr_2;  }

   inline static void set_prev_in_group(node_ptr n, node_ptr prev)
   {  n->node_ptr_2 = prev;  }

   inline static std::size_t get_hash(const const_node_ptr & n)
   {  return n->size_t_1;  }

   inline static void set_hash(const node_ptr & n, std::size_t h)
   {  n->size_t_1 = h;  }
};
...
{
   template <class T>
   static void function_not_available_for_any_hooks(typename std::enable_if<std::is_same_v<T, bool>>::type)
   {}
   ...
   inline static void init(const node_ptr & node) BOOST_NOEXCEPT
   ...
   inline static bool unique(const const_node_ptr & node) BOOST_NOEXCEPT
   {  return !node->node_ptr_1; }
