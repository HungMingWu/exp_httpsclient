   ...
   :  public slist_node_traits<VoidPointer>
{
   ...
   inline static node_ptr get_next(const const_node_ptr & n) noexcept
   {  return pointer_traits<node_ptr>::static_cast_from(n->next_);  }

   inline static void set_next(node_ptr n, node_ptr next) noexcept
   {  n->next_ = next;  }

   inline static node_ptr get_prev_in_group(const const_node_ptr & n) noexcept
   {  return n->prev_in_group_;  }

   inline static void set_prev_in_group(node_ptr n, node_ptr prev) noexcept
   {  n->prev_in_group_ = prev;  }

   inline static std::size_t get_hash(const const_node_ptr & n) noexcept
   {  return n->hash_;  }

   inline static void set_hash(const node_ptr & n, std::size_t h) noexcept
   {  n->hash_ = h;  }
};
   ...
   : public circular_slist_algorithms<NodeTraits>
{
   ...
   inline static void init(typename base_type::node_ptr n) noexcept
   {
      base_type::init(n);
      group_algorithms::init(n);
   }

   inline static void init_header(typename base_type::node_ptr n) noexcept
   {
      base_type::init_header(n);
      group_algorithms::init_header(n);
   }

   inline static void unlink(typename base_type::node_ptr n) noexcept
   {
      base_type::unlink(n);
...
template <typename ...Options>
using make_unordered_set_base_hook_t = typename make_unordered_set_base_hook<Options...>::type;
...
class unordered_set_base_hook
   :  public make_unordered_set_base_hook_t<Options...>
{
   ...
   //!   user to do some additional work. \c swap can be used to emulate
   //!   move-semantics.
   unordered_set_base_hook& operator=(const unordered_set_base_hook& ) noexcept;
   ...
   //!
   //! <b>Throws</b>: Nothing.
   void swap_nodes(unordered_set_base_hook &other) noexcept;
