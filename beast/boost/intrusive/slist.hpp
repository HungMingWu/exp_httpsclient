...
#else
template<class ValueTraits, std::size_t BoolFlags, typename HeaderHolder>
#endif
class slist_impl
{
   //Public typedefs
   ...
   /// @cond
   private:
   typedef detail::size_holder<constant_time_size>        size_traits;
   ...
   static_assert(!(linear && value_traits::link_mode == link_mode_type::auto_unlink));
   //A list with cached last node is incompatible with auto-unlink hooks!
   static_assert(!(cache_last && value_traits::link_mode == link_mode_type::auto_unlink));
   ...
   {  return this->get_last_node(std::bool_constant<cache_last>());  }

   const_node_ptr get_last_node() const
   {  return this->get_last_node(std::bool_constant<cache_last>());  }

   void set_last_node(const node_ptr &n)
   {  return this->set_last_node(n, std::bool_constant<cache_last>());  }

   static node_ptr get_last_node(std::false_type)
   {
   ...
   static void set_last_node(const node_ptr &, std::false_type)
   {
      //This function shall not be used if cache_last is not true
      BOOST_INTRUSIVE_INVARIANT_ASSERT(cache_last);
   }

   node_ptr get_last_node(std::true_type)
   {  return node_ptr(data_.root_plus_size_.last_);  }

   const_node_ptr get_last_node(std::true_type) const
   {  return const_node_ptr(data_.root_plus_size_.last_);  }

   void set_last_node(const node_ptr & n, std::true_type)
   ...
   slist_impl( const node_ptr & f, const node_ptr & before_l
             , std::size_t n, const value_traits &v_traits = value_traits())
      :  data_(v_traits)
   {
      if(n){
   ...
   slist_impl(const slist_impl&) = delete;
   slist_impl& operator=(const slist_impl&) = delete;
   ...
   //!
   //! <b>Note</b>: This function is present only if cached_last<> option is true.
   inline const_iterator clast() const noexcept
   { return const_iterator(this->get_last_node(), this->priv_value_traits_ptr()); }
   ...
   //!
   //! <b>Complexity</b>: Constant.
   inline static slist_impl &container_from_end_iterator(iterator end_iterator) noexcept
   ...
   //!
   //! <b>Complexity</b>: Constant.
   inline static const slist_impl &container_from_end_iterator(const_iterator end_iterator) noexcept
   {  return slist_impl::priv_container_from_end_iterator(end_iterator);   }
   ...
   void swap(slist_impl& other)
   {
      if constexpr (cache_last) {
         ...
         if constexpr (linear) {
             node_algorithms::swap_trailing_nodes(this->get_root_node(), other.get_root_node());
         }
         else {
   ...
   void shift_backwards(std::size_t n = 1) noexcept
   {
       if constexpr (linear) {
           auto [first, second] =
           ...
           if (first) {
               node_traits::set_next(this->get_root_node(), first);
               if constexpr (cache_last) {
                   this->set_last_node(second);
               }
           }
       }
       else {
   ...
   //!
   //! <b>Note</b>: Does not affect the validity of iterators and references.
   void shift_forward(std::size_t n = 1) noexcept
   {
       if constexpr (linear) {
           auto [first, second] =
   ...
   //! <b>Note</b>: Invalidates the iterators (but not the references) to the
   //!   erased element.
   iterator erase_after(const_iterator before_f, const_iterator l, std::size_t n) noexcept
   {
   ...
   //! <b>Note</b>: Invalidates the iterators (but not the references) to the
   //!   erased element.
   iterator erase(const_iterator f, const_iterator l, std::size_t n) noexcept
   ...
   {
      static_assert(((!cache_last)&&(!constant_time_size)&&(!stateful_value_traits)));
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of this
   //!   list. Iterators of this list and all the references are not invalidated.
   void splice_after(const_iterator prev_pos, slist_impl &x, const_iterator before_f, const_iterator before_l, std::size_t n) noexcept
   {
   ...
   //! <b>Note</b>: Iterators of values obtained from list x now point to elements of this
   //!   list. Iterators of this list and all the references are not invalidated.
   void splice(const_iterator pos, slist_impl &x, const_iterator f, const_iterator l, std::size_t n) noexcept
   ...
   //!
   //! <b>Warning</b>: Experimental function, don't use it!
   void incorporate_after(const_iterator prev_pos, const node_ptr & f, const node_ptr & before_l) noexcept
   {
      if constexpr (constant_time_size)
   ...
   //!
   //! <b>Warning</b>: Experimental function, don't use it!
   void incorporate_after(const_iterator prev_pos, const node_ptr & f, const node_ptr & before_l, std::size_t n) noexcept
   {
      if(n){
         BOOST_INTRUSIVE_INVARIANT_ASSERT(n > 0);
         BOOST_INTRUSIVE_INVARIANT_ASSERT
            (std::size_t(boost::intrusive::iterator_distance
      ...
      >::type
{
   ...
   inline slist( const node_ptr & f, const node_ptr & before_l
             , std::size_t n, const value_traits &v_traits = value_traits())
   ...
   inline slist(slist&& x) : Base(std::move(x)) {}

   inline slist& operator=(slist&& x)
   {  return static_cast<slist &>(this->Base::operator=(std::move(x)));  }
   ...
   template <class Cloner, class Disposer>
   inline void clone_from(slist&& src, Cloner cloner, Disposer disposer)
   {  Base::clone_from(std::forward<slist>(src), cloner, disposer);  }

   inline static slist &container_from_end_iterator(iterator end_iterator) noexcept
   {  return static_cast<slist &>(Base::container_from_end_iterator(end_iterator));   }

   inline static const slist &container_from_end_iterator(const_iterator end_iterator) noexcept
